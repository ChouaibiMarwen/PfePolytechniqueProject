...
@RestController
@CrossOrigin
@RequestMapping(value = "/api/v1/sub_dealer")
public class SubDealerController extends BaseController {
    @Autowired
    ...
    public ResponseEntity<users> add_sub_dealer(@RequestBody SupplierSingUpRequest request) throws IOException, InterruptedException, MessagingException {
        ...
        UserAction action = new UserAction(
                UserActionsEnum.SUB_DEALER_MANAGEMENT,
                currentuser
        );
    ...
    @PreAuthorize("hasRole('ADMIN') or hasRole('SUB_ADMIN')  or hasRole('SUB_DEALER') or hasRole('SUB_SUB_DEALER')")
    @ApiOperation(value = "get all sub dealers without pagination", notes = "Endpoint to get suppliers")
    @ApiResponses(value = {
