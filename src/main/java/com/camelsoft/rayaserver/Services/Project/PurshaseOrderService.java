...
*/
    ...
    public DynamicResponse findAllPurchaseOrderPgByVehicleAndDateAndPurchaseOrderStatusAndSupplierforadmin(int page, int size, Long idVehicles, PurshaseOrderStatus status, Date date, Supplier supplier , users subadmin) {
        try {
            Pageable pageable = PageRequest.of(page, size);
            CriteriaBuilder cb = entityManager.getCriteriaBuilder();
            CriteriaQuery<PurshaseOrder> cq = cb.createQuery(PurshaseOrder.class);
            ...
            if (idVehicles != null) {
                ...
                predicates.add(vehicleIdPredicate);
            }
